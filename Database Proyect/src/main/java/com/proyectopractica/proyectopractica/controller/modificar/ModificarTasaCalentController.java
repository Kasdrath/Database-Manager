...
public class ModificarTasaCalentController {
    ...
    @FXML
    private TextField modificartasatf;
    ...
    @FXML
    void modificarTasa(ActionEvent event) throws IOException {
        if (modificartasatf.getText().trim().isEmpty()) { //VER QUE TEXTFIELD NO ESTE VACIO
            ...
            try {
                ...
                while (rs.next()){
                    if(modificartasatf.getText().equals(rs.getString("tasa_calent"))){ //COMPRUEBA QUE NO AGREGE UN NOMBRE QUE YA ESTE
                        FXMLLoader fxmlLoader = new FXMLLoader(Inicio.class.getResource("Error.fxml"));
                        Scene scene = new Scene(fxmlLoader.load(), 356, 113);
                        Stage stage = new Stage();
                        stage.setTitle("ERROR");
                ...
                stmt.executeUpdate("update tasa_calent set tasa_calent ="+"'"+modificartasatf.getText()+"'"+"where id_tasa_calent ="+idfam+";");
